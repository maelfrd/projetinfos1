...
{                                                           
    ...
    if (fgets(buf, sizeof(buf), stdin) == NULL)             /* Lit une ligne depuis clavier */
        return 'x';                                        /* En cas d’erreur ou de fin de fichier (EOF), le programme s’arrête */
                                                            
    if (buf[0] == '\n')                                     /* Si l'utilisateur appuie juste sur la touche Entree */
        return '\n';                                        /* Retourne caractere sur une nouvelle ligne */
                                                            
    for (i = 0; buf[i] != '\0'; i++)                        /* Parcourt chaque caractere du buffer */
        if (buf[i] != ' ' && buf[i] != '\t' && buf[i] != '\n')  /* Si il n'y a pas  d'espace, tab, newline */
            return buf[i];                                  /* Retourne le premier caractere valide */
...
{                                                         
    if (cmd == 'x' || cmd == 'X') {                        
        jeu->en_cours = 0;                                  /* Met fin à la partie */
        return;                                             /* Sort de la fonction */
    }                                                      
                                                           
    if (cmd == 'z' || cmd == 'Z') {                        
        if (jeu->curseur_x > 0)                             /* Si pas en haut du plateau */
            jeu->curseur_x--;                               /* Deplacer curseur vers le haut */
    }                                                       
    else if (cmd == 's' || cmd == 'S') {                   
        if (jeu->curseur_x < jeu->lignes - 1)               /* Si pas en bas du plateau */
            jeu->curseur_x++;                               /* Deplacer curseur vers le bas */
    }                                                       
    else if (cmd == 'q' || cmd == 'Q') {                   
        if (jeu->curseur_y > 0)                             /* Si pas a gauche du plateau */
            jeu->curseur_y--;                               /* Deplacer curseur vers la gauche */
    }                                                       
    else if (cmd == 'd' || cmd == 'D') {                   
        if (jeu->curseur_y < jeu->colonnes - 1)             /* Si pas a droite du plateau */
            jeu->curseur_y++;                               /* Deplacer curseur vers la droite */
    }                                                       
    else if (cmd == 'p' || cmd == 'P') {                   
        if (!jeu->selection) {                              /* Si aucune selection active */
            jeu->selection = 1;                             /* On active la selection */
            jeu->select_x = jeu->curseur_x;                 /* Sauvegarde de la position X selectionnee */
            jeu->select_y = jeu->curseur_y;                 /* Sauvegarde de la position Y selectionnee */
        }                                                   
        else if (jeu->curseur_x == jeu->select_x &&         /* Si le curseur est sur case selectionnee */
                 jeu->curseur_y == jeu->select_y) {         /* (meme X et meme Y) */
            jeu->selection = 0;                             /* On desactive la selection */
