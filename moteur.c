...
int verifierAlignementDepuis(char *plateau[], int lignes, int colonnes, 
    ...
    // Compte à gauche
    for(int j = y - 1; j >= 0; j--) {
        if(strcmp(plateau[x * colonnes + j], bonbon) == 0) {
            compte_h++;
            debut_h = j;
        } else {
            break;
    ...
    // Marque l'alignement horizontal si >= 3
    if(compte_h >= 3) {
        for(int j = debut_h; j <= fin_h; j++) {
    ...
    // Compte en haut
    for(int i = x - 1; i >= 0; i--) {
        if(strcmp(plateau[i * colonnes + y], bonbon) == 0) {
            compte_v++;
            debut_v = i;
        } else {
            break;
    ...
    // Marque l'alignement vertical si >= 3
    if(compte_v >= 3) {
        for(int i = debut_v; i <= fin_v; i++) {
...
int permutationValide(char *plateau[], int lignes, int colonnes, 
    ...
    int *marques = calloc(total, sizeof(int));
    int valide = verifierAlignementDepuis(plateau, lignes, colonnes, x1, y1, marques) ||
...
void traiterAlignementsCascade(JeuState *jeu) {
    ...
    while(continuer_traitement) {
        ...
        int *marques = calloc(total, sizeof(int));
        int alignements_trouves = verifierTousAlignements(jeu->plateau, jeu->lignes, 
                                                          jeu->colonnes, marques);
        
        if(alignements_trouves) {
            supprimerBonbonsMarques(jeu->plateau, jeu->lignes, jeu->colonnes, 
                                   marques, jeu->emojis, jeu->nbemoji, NB_TYPES_BONBONS);
            faireTomber(jeu->plateau, jeu->lignes, jeu->colonnes);
            remplirCasesVides(jeu->plateau, jeu->lignes, jeu->colonnes, 
                            jeu->emojis, NB_TYPES_BONBONS);
            usleep(300000);
        } else {
